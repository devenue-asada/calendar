...
class Calendar:
    ...
    def __init__(self, year, month):
        ...
        # 余り日数
        self.rem_days = self.count % week
        # 初週の日数
        self.first_week_days = week - self.count_beginning_blank()
    ...
    def count_week(self):
        # 最低の週数
        min_count = math.floor(self.count / week)
    ...
    # Arguments
    # Return 年(int)
    def count_beginning_blank(self):
    ...
    def print_calendar(self):
        ...
        count_last_week_blank = 0 if last_week == 0 else week - last_week if last_week >= 0  else -last_week
        for i in range(count_last_week_blank):
            arr.append(blank)
        # 改行
        calendar = []
        for i, day in enumerate(arr):
            calendar.append(f'{day}\n') if (int(i + 1) % week) == 0 else calendar.append(day)
        # 描画
...
def create_calendar(year,count_range):
